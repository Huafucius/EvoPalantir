...
| messages        | LangChain BaseMessage[] | 下一次调用 LLM 时传入的完整消息列表      |
...
每条投影出的 LangChain message 在 `additional_kwargs["aos"]` 中携带最小 provenance：

```python
message.additional_kwargs["aos"] = {
    "sourceMessageId": "<SessionHistory message id>",
    "sourcePartId": "<part id>",       # 可选
    "kind": "<投影类型>",
}
```
...
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| role=`user`、origin=`human`、TextPart                                                               | `HumanMessage(content=text)`                                                                                                         |
| role=`assistant`、origin=`assistant`、TextPart（无工具调用）                                        | `AIMessage(content=text)`                                                                                                            |
| role=`assistant`、origin=`assistant`，含 ToolBashPart（state=`output-available` 或 `output-error`） | `AIMessage(content="", tool_calls=[{id, name:"bash", args:{command,cwd?}}])` + `ToolMessage(content=visibleResult, tool_call_id=id)` |
| role=`user`、origin=`aos`、SkillLoadPart                                                            | `SystemMessage(content=f"[[AOS-SKILL name={name} owner={ownerType}]]\n{skillText}")`                                                 |
| role=`user`、origin=`aos`、CompactionMarkerPart                                                     | `HumanMessage(content="What did we do so far?")`                                                                                     |
| role=`assistant`、origin=`aos`、summary=`true`、TextPart                                            | `AIMessage(content=<summaryText>)`                                                                                                   |
| role=`user`、origin=`aos`、InterruptPart                                                            | `SystemMessage(content=f"[[AOS-INTERRUPT reason={reason}]]")`                                                                        |
| BootstrapPart（任意消息）                                                                           | 不投影                                                                                                                               |
| state=`input-streaming` 或 `input-available` 的 ToolBashPart                                        | 不投影（调用未完成）                                                                                                                 |

**补充说明：**

- 一条含有 ToolBashPart 的 assistant 消息投影为**两条** LangChain messages：一条 AIMessage（携带 tool_call），一条 ToolMessage（携带 visibleResult）。sourcePartId 两条都指向同一 ToolBashPart 的 id，以 kind 区分。
- 若一条 assistant 消息同时有 TextPart 和 ToolBashPart，TextPart 内容置入 AIMessage.content，ToolBashPart 置入 AIMessage.tool_calls。
- SkillLoadPart 投影为 SystemMessage，按其 seq 顺序插入，不聚合到列表头部。
- CompactionMarkerMessage 与 CompactionSummaryMessage 合在一起投影为 HumanMessage + AIMessage。
...
3. 应用投影规则生成 LangChain BaseMessage[]
...
| SessionContext 自动调度策略              | 不内置                                    | 由 skill / plugin 自行实现      |
