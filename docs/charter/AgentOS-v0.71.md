...
v0.71 将 trace 明确对齐到 AI SDK 的 `UIMessage[]` 风格。AI SDK 把 `UIMessage` 定义为应用状态的 source of truth，顶层角色只有 `system | user | assistant`，并允许在 `metadata` 与 `data-*` parts 中承载自定义语义。AOS 的全部扩展，都必须优先通过这两个位置表达，不得随意改写 message 顶层骨架。
...
传统 Agent Skills 所说的 execution，指的是 agent 读入说明后按说明做事；它并不等于 AOS 的 `start`。`start` 是 AOS 在传统 skill 之上增加的生命周期启动语义。正因如此，AOS 才能把 skill 从单纯的 instruction package 扩展为既可注入文本、又可生成运行实例的能力单元。
...
这四者的关系是：生命周期是时间主轴，事件是主轴上的事实，钩子是事实发生时允许介入的插槽，阶段是运行中的瞬时局部状态。生命周期不是 hooks；但 hooks 依附于生命周期。事件也不等于运行轨迹：被 bus 分发的是运行时事件，被 trace 保存的是会话层面的持久事实。
...
interface RunningSkillInstance {
  ...
  startedAt: string;
  hooks: ControlHookName[];
  lastError?: string;
}
```

有些运行中的 skill 还会请求控制面启动受管资源，例如全栈 app、后台 service 或 worker。这些资源属于运行时结构，不构成新的系统本体。

```typescript
type RuntimeResourceKind = 'app' | 'service' | 'worker';
...
interface ManagedRuntimeResource {
  resourceId: string;
...
`ManagedRuntimeResource` 的意义在于：skill 可以拥有运行中的外部能力，但 AOS 不必因此引入第五本体。资源是运行期可被启动、查询、停止的对象；skill 仍然是能力抽象，实例仍然是生命周期载体，控制面仍然是唯一正式治理面。

### 5.3 Event、Bus 与监听拓扑

仅有生命周期状态与运行实例还不够。系统还需要回答：运行中发生的事实如何传播，谁有资格监听，哪些东西只是观察，哪些东西参与主控制流。

AOS 在运行时维护一条统一的事件流。事件是运行时事实，bus 是这些事实的分发机制。

```typescript
interface AOSEvent<T = unknown> {
  type: string; // dotted name, e.g. "session.started"
  scope: Scope;
  timestamp: string;
  agentId?: string;
  sessionId?: string;
...
interface EventSubscription {
  ...
  scope: Scope;
  ownerId: string;
  patterns?: string[]; // omitted means subscribe all visible events
}

interface AOSBus {
  publish(event: AOSEvent): void;
  subscribe(input: EventSubscription): Unsubscribe;
}
```

这里的 `AOSBus` 是核心运行时形式，但不是持久化消息队列。它不进入 trace，不替代控制块，也不负责 TPM / RPM 的削峰填谷。它的职责只有一个：把已经发生在运行时的事实，按 scope 与可见性规则传播给运行中的 skill 实例。

因此，`trace`、`bus` 与执行队列必须严格分离：

- `trace` 负责持久化会话事实。
- `bus` 负责传播运行时事件。
- `queue` 负责执行调度、并发控制与限流。
...
### 5.4 运行入口、Hook 与受限 AosSDK

事件能传播之后，系统还必须说明实例在何处介入控制流，以及它们以什么能力边界与控制面交互。

运行入口遵循 OpenCode 风格的 JS/TS 模块约定，但在能力边界上比 OpenCode 收得更紧：

```typescript
type SkillRuntimeInit = (
  ctx: SkillRuntimeContext,
) => Promise<Partial<ControlHookTable>>;
...
type ControlHookTable = {
  [K in ControlHookName]: (payload: ControlHookPayloadMap[K]) => Promise<void>;
};
```

这里的 `ctx.aos` 在语义上与公共 `AosSDK` 同构，但在实际暴露时必须按 scope 与策略裁剪。运行中的 skill 实例拿到的是同一控制面的受限客户端，而不是无条件拥有全部操作的万能句柄。

与 OpenCode 的关键区别值得说明。OpenCode 向 plugin 交出完整的 SDK 客户端与 Bun Shell 直接执行能力。AOS 刻意收窄为 `{ scope, ownerId, skillName, aos }`。`SkillRuntimeContext` 不暴露 AOSConfig、ACB 或 SCB 原始对象，也不提供直接执行 shell 的能力；运行中的 skill 实例只能通过 `AosSDK` 与控制面交互。收窄的理由是治理统一：如果 skill 实例可以绕过控制面直接修改系统本体，那么“凡系统控制，皆经 AOS Control Plane 完成”就成了空话。

在这个前提下，skill 实例仍然可以拥有很强的现实能力，但这些能力必须由控制面授予。例如，某个 skill 可以通过受限 `AosSDK` 请求控制面启动一个受管的 app、service 或 worker；它不能直接拉起进程，却可以合法地请求系统为自己创建运行资源。这样，AOS 既保住了治理边界，又不牺牲全栈 skill 的能力密度。
...
本章固定的 `HookName` 只指控制流 hooks。它们的名称与 payload 固定如下：

```typescript
type ControlHookName =
  | 'aos.started'
  | 'aos.stopping'
  | 'agent.started'
  | 'agent.closed'
...
interface ControlHookPayloadMap {
  'aos.started': {
    cause: 'boot' | 'recover';
    timestamp: string;
    catalogSize: number;
...
- 通知型 hooks（`session.started`、`session.error`、`session.interrupted`、`session.closed`、`agent.started`、`agent.closed`、`resource.started`、`resource.stopping`、`resource.error`）：近层 → 远层
- `aos.*`：仅 system
...
```typescript
type ExecutionKind = 'compute' | 'tool' | 'compaction' | 'resource-op';
...
`ExecutionTicket` 是运行期调度对象，不进入控制块，也不进入 trace。它只用于把“该做的事”放入执行队列，并在资源允许时真正运行。

默认执行规则应满足三点：

1. **单 Session 单活跃执行链。** 同一 `sessionId` 在同一时刻最多只应有一条主执行链，以避免一条会话被并发 compute / tooling 拉成难以解释的状态。
2. **跨 Session 可并行。** 不同 Session 之间可以并发推进，但要受 provider 预算、主机并发与策略限制。
3. **限流属于调度，不属于 bus。** TPM / RPM、provider budget、local concurrency 与 backpressure 都由 scheduler 决定，不由事件总线承担。

因此，`queued` phase 具有明确意义：这并不表示会话已经停止，而只是说明它已经具备运行资格，正在等待调度器授予执行时机。

队列的默认实现可以是进程内、内存型、类型化的执行队列；未来若系统进入分布式形态，可以把底层替换为更强的 transport 或 worker backend，但这不改变本宪章对 queue 的逻辑定义。
...
5. 启动 system 级默认 `start` skill 实例，并建立 system 级事件订阅。
6. 发出 `aos.started`。
...
Agent 创建或恢复时，激活顺序依次是：读取或创建 ACB；预热该 Agent 持有的默认 load skill 文本缓存；对该 Agent 中 `start: "enable"` 的条目做一次 reconcile，确保每个 `(scope="agent", ownerId=agentId, name)` 且被声明为默认启动的 skill 恰有一个运行实例；建立其 agent 级事件订阅；最后发出 `agent.started`。

Session 创建或恢复时，顺序则转入 bootstrap。由于 Session 既是事务单元，也是 trace 与 prompt 重建的入口，它的启动链比 Agent 更长，也更接近用户真正能感知的运行过程。

### 6.2 Session Bootstrap 与默认 Skill 消费

Session 创建或恢复时，流程按以下顺序执行：

1. 创建或读取 SCB；若新建，初始 `status = "initializing"`。
2. 预热该 Session 持有的默认 load skill 文本缓存。
3. 对该 Session 中 `start: "enable"` 的条目做一次 reconcile，并建立 session 级事件订阅。
4. 进入 `bootstrapping` phase。
5. 向 trace 追加 `data-bootstrap { phase: "begin", reason }` 消息。
6. 发出 `session.bootstrap.before`。
7. 解析默认 load 集合，并按顺序向 trace 追加 `data-skill-load` 消息。
...
`session.bootstrap.before` 与 `session.bootstrap.after` 分别标志默认注入的开始与完成，属于控制流 hook 语义；`data-bootstrap` begin/done marker 的职责则是为恢复协议提供轨迹记录。两者服务于不同目的，不能相互替代。
...
默认 `start` 的消费逻辑与默认 `load` 不同。它不做跨层覆盖，而是在各自作用域生命周期起点独立消费各层控制块中的 `start` 条目。system 级实例在 AOS 启动阶段 reconcile；agent 级实例在 Agent 创建或恢复时 reconcile；session 级实例在 Session bootstrap 开始前 reconcile。二者共享 `DefaultSkillEntry`，但消费时机并不相同。

### 6.3 上下文投影、Session Loop 与显式 Load
...
### 6.4 Compaction 与 Reinject

compaction 是 Session 的内建维护原语。它的目标不是改变事实，而是在不丢失可恢复性的前提下，压缩模型需要重新看到的历史区间，并为后续上下文恢复重新注入默认 load skill。

触发 compaction 后，流程按固定顺序执行：
...
compaction 只压缩历史并重注入默认 load skill，绝不重新启动任何 running skill instance，也绝不重新创建既有 runtime resource。若 compaction 同时被视为“上下文重建”和“作用域重新开始”，system、agent 级 start skill 与其拥有的受管资源就会被错误地频繁重启，这与它们本应绑定的生命周期边界相悖。

### 6.5 关闭、停止与崩溃恢复

关闭流程按作用域分三个层次，各自独立执行后再向上收拢。

Session 关闭时，依次停止该 Session 持有的全部运行 skill 实例与其拥有的全部受管 runtime resources，将 `SCB.status` 置为 `"closed"` 并写入 `closedAt`，然后发出 `session.closed`。

Agent 关闭时，依次停止该 Agent 持有的全部运行 skill 实例与其拥有的全部受管 runtime resources，将 `ACB.status` 置为 `"closed"` 并写入 `closedAt`，然后发出 `agent.closed`。

AOS 停止时，流程稍有不同：先发出 `aos.stopping`，再停止全部 system 级运行 skill 实例与 system 级受管 runtime resources，释放运行态注册表、事件订阅与缓存引用，最后进入 `stopped`。
...
interface ControlSchema {
  ...
  'resource.list': {
    args: { scope?: Scope; ownerId?: string };
    result: ManagedRuntimeResource[];
  };
  'resource.start': {
    args: { scope: Scope; ownerId?: string; spec: RuntimeResourceSpec };
    result: ManagedRuntimeResource;
  };
  'resource.get': {
...
interface AosSDK {
  ...
  resource: {
    list(args: {
      scope?: Scope;
      ownerId?: string;
    }): Promise<AosResponse<ManagedRuntimeResource[]>>;
    start(args: {
      scope: Scope;
      ownerId?: string;
    ...
    stop(args: {
      resourceId: string;
    }): Promise<AosResponse<{ resourceId: string }>>;
  };
}
```
...
| `aos resource start --scope session`                       | `aos.resource.start({ scope: "session", spec })`                                       |
...
| 运行时 bus 事件                    | 不直接进入；仅当它们形成会话事实并被写入 trace 时才持久化 |
| 实例内部的 HTTP / shell 调用       | 不进入（除非通过控制面影响了 session）                    |
| 宿主层面的缓存命中 / 失效          | 不进入                                                    |
...
有几个常见边界案例值得单独厘清：subagent 是否构成新本体，memory 与 channel 是否属于 AOS 内建子系统，以及全栈 app 是否会引入新的系统层。

AOS 不把 subagent 当作第五本体。若一个 Agent 或一个人类通过 `agent.create` 创建了新的 ACB，那么产生的就是一个新的 Agent。它是否短命、是否只为子任务而生、是否由另一个 Agent 发起，都不改变它作为 Agent 的结构事实。创建来源通过 `ACB.createdBy` 记录；同时，创建动作本身也作为 tool-call / tool-result 留存在发起创建的 Session 的 trace 中。绝大多数“临时跑一个子任务”的场景，甚至不需要创建新 Agent；在同一个 Agent 下新建一个 Session 就够了。

memory、channel、notion、飞书、模拟社交平台，也都不构成新的 AOS 本体。它们都只是不同的 skill。某个 memory system 可以被替换为另一种 memory skill，某个 channel integration 也可以被另一种 channel skill 取代；AOS 提供的是统一的生命周期、控制面、trace、bus 与运行资源托管，而不是把这些具体能力写死成系统内建子模块。

app、service 与 worker 也不是第五本体。它们是由 running skill instance 通过控制面请求创建的受管 runtime resources。它们存在于运行时、受 scope 与 owner 约束、可被启动与停止，但不因此进入四层 ontology。
...
Agent OS 是一个管理认知主体、会话运行轨迹、skill 注入与 skill 运行实例的认知操作系统。其本体层由 Compute Unit、Agent、Session、Skill 四层构成，静态控制结构由 AOSConfig、ACB、SCB 承载。会话运行轨迹以 AI SDK 风格的 `UIMessage[]` 记录，`metadata` 与 `data-*` parts 承载 AOS 语义，`convertToModelMessages` 完成模型投影。skill 的两种行为——load 对应上下文注入，start 对应实例化启动——由生命周期驱动，hooks 是实例在事件点上介入的正式机制，event bus 则负责传播运行时事实。整个系统以 AOS Control Plane 作为唯一正式治理面，CLI、SDK 与 UI 均作为其客户端存在。
