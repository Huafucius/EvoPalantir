...
| 作用域  | 归档动作                                                                                        | 最终状态                |
| ------- | ----------------------------------------------------------------------------------------------- | ----------------------- |
| Session | 停止该 Session 的全部运行实例与其拥有的全部受管资源，写入 `archivedAt`，发出 `session.archived` | `SCB.status = archived` |
| Agent   | 停止该 Agent 的全部运行实例与其拥有的全部受管资源，写入 `archivedAt`，发出 `agent.archived`     | `ACB.status = archived` |
...
| `agent.archive` | `agentId`      | `revision`     | 是         | 停止该 Agent 的运行实例与资源     | 归档 Agent |
...
| `session.archive`   | `sessionId`                       | `revision`                  | 是         | 停止该 Session 的运行实例与资源 | 归档 Session     |
