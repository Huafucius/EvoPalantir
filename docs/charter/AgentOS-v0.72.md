...
flowchart TD
    ...
    SK -->|load into context| SE
    SK -->|start as instance| AG
    SK -->|start as instance| SE
    SK -->|usable by| CU
```
...
本章只讨论应被持久化、应被恢复、应被当作系统依据的结构。运行时缓存、hook 注册表、running skill instance、event bus 与 execution queue 都不属于本章，它们将在第五章讨论。
...
用途：AOS 对 skill 静态语义的归一化结果。AOS 只从磁盘上的 `SKILL.md` frontmatter 中读取 `name`、`description` 与 `metadata.aos-plugin`，并将第三项归一化为 `aosPlugin`。

| 字段          | 取值 / 类型 | 必填 | 可变 | 含义                                      |
| ------------- | ----------- | ---- | ---- | ----------------------------------------- |
| `name`        | string      | 是   | 否   | skill 名；在整个 AOS 实例内唯一           |
| `description` | string      | 是   | 否   | 给 Compute Unit 看的简短说明              |
| `aosPlugin`   | string      | 否   | 否   | 由磁盘上的 `metadata.aos-plugin` 解析而来 |
...
用途：某一层控制块中的默认 skill 条目。它只保存默认行为：当某个作用域启动或重建上下文时，哪些 skill 应默认参与 `load`，哪些 skill 应默认参与 `start`。

| 字段    | 取值 / 类型          | 必填 | 可变 | 含义                                      |
| ------- | -------------------- | ---- | ---- | ----------------------------------------- |
| `name`  | string               | 是   | 是   | skill 名                                  |
| `load`  | `enable` / `disable` | 否   | 是   | 该 skill 是否参与默认上下文注入           |
| `start` | `enable` / `disable` | 否   | 是   | 该 skill 是否在作用域生命周期起点默认启动 |
...
| 字段            | 取值 / 类型                  | 必填 | 可变 | 含义                                     |
| --------------- | ---------------------------- | ---- | ---- | ---------------------------------------- |
| `schemaVersion` | string                       | 是   | 否   | 文档与持久化结构版本；当前为 `aos/v0.72` |
...
| `status`        | `initializing` / `ready` / `closed` | 是   | 是   | Session 生命周期状态      |
...
- 运行失败分别体现在实例状态、trace 事实、`session.error` hook 与控制面错误返回中。
...
v0.72 将 trace 明确对齐到 AI SDK 的 `UIMessage[]` 风格。AI SDK 把 `UIMessage` 定义为应用状态的 source of truth，顶层角色只有 `system | user | assistant`，并允许在 `metadata` 与 `data-*` parts 中承载自定义语义。AOS 的全部扩展，都通过这两个位置表达。
...
| part         | 关键字段                          | 用途                              |
| ------------ | --------------------------------- | --------------------------------- |
| `skill-load` | `cause`, `scope`, `name`, `text`  | 默认注入或 reinject 的 skill 文本 |
...
这样定义的目的，是为系统建立稳定的能力边界。后文无论提到 instruction text、运行入口、默认 load、默认 start，谈论的都还是同一个 skill，只是时间语义和消费方式不同。
...
| skill       | `load` | `start` | 含义                                                                                    |
| ----------- | ------ | ------- | --------------------------------------------------------------------------------------- |
| `profile`   | 是     | 否      | 只注入 instruction text，提供角色设定与偏好                                             |
| `memory`    | 是     | 是      | 既注入 instruction text，又把分散在各 Session 中的历史沉淀为可跨 session 检索的长期记忆 |
| `telemetry` | 否     | 是      | 不进 prompt，只作为运行实例订阅 hooks 做监测                                            |
...
`SKILL.md` 是 skill 的最小入口，也是 Compute Unit 真正会被注入的正文说明书。磁盘上的 frontmatter 可以带有各种字段，但 AOS 解析时只抽取 `name`、`description` 与 `metadata.aos-plugin` 三项语义，并归一化为第三章定义的 `SkillManifest`。示例：
...
磁盘格式中的运行入口固定写在 `metadata.aos-plugin`。AOS 解析后把它归一化为 `aosPlugin`。也正是借助这个扩展点，skill 才同时拥有 `load` 与 `start` 两种能力面。

### 4.4 Discover、Load 与 Start

AOS 只承认 skill 的三类核心动作。

**discover：** 系统扫描 `skillRoot`，建立 `SkillManifest` 并从中投影出 `SkillCatalog`。Compute Unit 只看到渲染出的 catalog 字符串，据此判断是否需要加载某个 skill。这一层对应官方 Skills 的 progressive disclosure 模型中 Level 1。

**load：** 把 skill 的 `SKILL.md` 完整正文带入模型上下文。显式 load 通过 bash 调 `aos skill load <name>`；默认 load 由系统在上下文重建时自动注入。无论哪种方式，结果都落入 trace。

**start：** 根据 `aosPlugin` 创建运行中的 skill 实例。该实例拥有生命周期，可以注册 hooks、订阅事件、通过 AOS SDK 与控制面交互，并在需要时请求控制面启动受管的 app、service 或 worker。

```mermaid
flowchart TD
    P[Skill package]
    M[SkillManifest / SkillCatalog]
    T[Skill text in trace]
    I[Running skill instance]
    R[Managed runtime resource]

    P -->|discover| M
    P -->|load| T
    P -->|start via aosPlugin| I
    I -->|request via control plane| R
```

在通用 skill 标准中，execution 指 agent 读入说明后按说明做事。AOS 在此基础上增加 `start`：它让 skill 不仅能提供说明书，还能拥有运行中的生命周期与正式的系统介入点。
...
这四者的关系是：生命周期是时间主轴，事件是主轴上的事实，钩子是事实发生时允许介入的插槽，阶段是运行中的瞬时局部状态。事件与运行轨迹分属两层：被 bus 分发的是运行时事件，被 trace 保存的是会话层面的持久事实。
...
| `ready`        | 已完成 bootstrap，可继续推进 | bootstrap 完成 | `closed` |
| `closed`       | 已终止                       | 关闭           | 无       |
...
| `text cache`           | 保存默认 `load` skill 的文本                      | 否         |
| `runtime module cache` | 保存运行入口模块引用                              | 否         |
| `instance registry`    | 保存运行中的 skill instance                       | 否         |
| `resource registry`    | 保存受管 runtime resources                        | 否         |

这些结构都是运行态派生结构，不写回控制块。

#### Running skill instance

用途：已经 `start` 的 skill 在运行时的实例视图。

| 字段         | 取值 / 类型                                               | 含义                                               |
| ------------ | --------------------------------------------------------- | -------------------------------------------------- |
| `instanceId` | string                                                    | 实例标识；由 `scope`、`ownerId` 与 `name` 组合而成 |
| `name`       | string                                                    | skill 名                                           |
| `scope`      | `system` / `agent` / `session`                            | 所属作用域                                         |
...
用途：由 running skill instance 通过控制面请求创建的受管资源，例如 app、service、worker。
...
| `scope`           | `system` / `agent` / `session`                            | 所属作用域                  |
| `ownerId`         | string                                                    | 拥有者标识                  |
| `ownerInstanceId` | string                                                    | 创建该资源的 skill instance |
...
- `metadata.aos-plugin` 解析结果变化：失效对应运行入口缓存条目。
- 既有 trace message 不可被重写。
- 已经运行中的 skill 实例继续使用启动时装入的运行模块，直到作用域结束或显式停止。
...
| `scope`     | `system` / `agent` / `session` | 事件归属作用域                               |
...
持久化会话事实由 trace 保存，执行排队由 queue 负责，AOSBus 承担的是运行时事件分发。

### 5.4 监听拓扑与可见性规则

在 AOS 中，真正的 listener 不是 Compute Unit，也不是 skill 包，而是 running skill instance。Compute Unit 只通过 prompt projection 与 `bash` 参与运行；控制块与 trace 是结构，也不订阅 bus。只有已经 `start` 的 skill 实例，才可能成为事件订阅者。

```mermaid
flowchart TD
    SYS[System listeners]
    AGT[Agent listeners]
    SES[Session listeners]
...
| 事件归属 scope | 可见给谁                                                       |
| -------------- | -------------------------------------------------------------- |
| `system`       | system listeners                                               |
| `agent`        | 对应 agent listeners、system listeners                         |
| `session`      | 对应 session listeners、所属 agent listeners、system listeners |

#### 解释

- system 级实例可以观察全局。
- agent 级实例可以观察本 agent 以及其下属 session 的事件。
- session 级实例只观察自己这个 session 的事件。
- sibling session 之间默认互不可见。

bus 的生产者天然是多对一的：控制面、session runtime、tool runtime、resource manager 与 running skill instance 都可以向同一条逻辑 bus 发布事件。bus 对 listener 的分发则是一对多：同一个事件可以被多个可见 scope 下的实例同时接收。
...
| `agent.closed`    | agent / system           | Agent 关闭后             | `agentId`, `timestamp`                       |
| `session.started` | session / agent / system | Session bootstrap 完成后 | `agentId`, `sessionId`, `cause`, `timestamp` |
| `session.closed`  | session / agent / system | Session 关闭后           | `agentId`, `sessionId`, `timestamp`          |
...
| hook                | 作用域           | 时机           | 关键字段                           |
| ------------------- | ---------------- | -------------- | ---------------------------------- |
| `resource.started`  | owner scope 向上 | 受管资源启动后 | `resourceId`, `kind`, `endpoints?` |
| `resource.stopping` | owner scope 向上 | 受管资源停止前 | `resourceId`, `kind`               |
| `resource.error`    | owner scope 向上 | 受管资源失败时 | `resourceId`, `kind`, `message`    |
...
运行中的 skill 实例拿到的是受 scope 与策略约束的 `AosSDK` 子集。它与公共控制面共享同一套语义，但只暴露当前实例被允许调用的操作集合。

#### SkillRuntimeContext

| 字段        | 取值 / 类型                    | 含义                                   |
| ----------- | ------------------------------ | -------------------------------------- |
| `scope`     | `system` / `agent` / `session` | 当前实例作用域                         |
| `ownerId`   | string                         | 当前实例拥有者                         |
| `skillName` | string                         | 当前 skill 名                          |
| `agentId`   | string                         | 当作用域为 `agent` 或 `session` 时存在 |
| `sessionId` | string                         | 当作用域为 `session` 时存在            |
| `aos`       | SDK 子集                       | 受 scope 约束的控制面客户端            |

skill 实例仍然可以拥有很强的现实能力，但这些能力必须由控制面授予。例如，某个 skill 可以通过受限 `AosSDK` 请求控制面启动一个受管的 app、service 或 worker；它不能直接拉起进程，却可以合法地请求系统为自己创建运行资源。
...
| `scope`           | `system` / `agent` / `session`                    | 执行归属作用域        |
...
因此，`queued` phase 具有明确意义：这并不表示会话已经停止，而只是说明它已经具备运行资格，正在等待调度器授予执行时机。
...
| `Initialization` | 载入运行入口、预热缓存、建立运行态注册表                   |
| `Activation`     | reconcile 默认 `start` skill、注册事件订阅、发出启动 hooks |
| `Runtime`        | 进入正常的 agent / session 创建、推进、压缩、关闭过程      |
| `Stopping`       | 发出停止 hooks、停止实例、释放运行态结构                   |
...
| 4    | 预热 system 级默认 `load` 文本缓存                      | 后续 bootstrap 可直接取文 |
| 5    | 启动 system 级默认 `start` 实例并建立 system 级事件订阅 | system 级运行时就绪       |
...
| ---- | ------------------------------------ |
| 1    | 读取或创建 `ACB`                     |
| 2    | 预热该 Agent 的默认 `load` 文本缓存  |
| 3    | 对 agent 级 `start` 条目做 reconcile |
| 4    | 建立 agent 级事件订阅                |
| 5    | 发出 `agent.started`                 |
...
| 2    | 预热 session 级默认 `load` 文本缓存                               | 无                                  | 无                         |
| 3    | 对 session 级 `start` 条目做 reconcile，并建立 session 级事件订阅 | 无                                  | 无                         |
| 4    | 进入 `bootstrapping` phase                                        | 无                                  | 无                         |
| 5    | 追加 begin marker                                                 | `data-bootstrap { phase: "begin" }` | 无                         |
| 6    | 开始默认注入                                                      | 无                                  | `session.bootstrap.before` |
| 7    | 解析默认 `load` 集合并注入 skill 文本                             | `data-skill-load` messages          | 无                         |
...
5. 对最终启用的每个 skill，按其生效来源作用域从对应文本缓存中取出正文；若缓存缺失，先补齐缓存，再继续注入。
...
默认 `start` 不做跨层覆盖，而是在各自作用域生命周期起点独立消费各层控制块中的 `start` 条目。system 级实例在 AOS 启动阶段 reconcile；agent 级实例在 Agent 创建或恢复时 reconcile；session 级实例在 Session bootstrap 开始前 reconcile。
...
| part         | 模型侧文本                                                    |
| ------------ | ------------------------------------------------------------- |
| `skill-load` | `[[AOS-SKILL name=<name> scope=<scope>]]` 后接正文            |
...
返回值是一个 `SkillLoadResult` JSON 对象：
...
这条操作不修改控制块，其结果通过 tool result 自然进入 trace。这样，临时查阅的说明书与未来的默认配置始终保持分离。

### 6.5 Compaction 与 Reinject

compaction 是 Session 的内建维护原语。它的目标是在不丢失可恢复性的前提下，压缩模型需要重新看到的历史区间，并为后续上下文恢复重新注入默认 `load` skill。
...
compaction 只压缩历史并重注入默认 `load` skill，绝不重新启动任何 running skill instance，也绝不重新创建既有 runtime resource。作用域生命周期与上下文重建由此保持清楚分离。
...
| 作用域  | 关闭动作                                                                                        | 最终状态              |
| ------- | ----------------------------------------------------------------------------------------------- | --------------------- |
| Session | 停止该 Session 的全部运行实例与其拥有的全部受管资源，写入 `closedAt`，发出 `session.closed`     | `SCB.status = closed` |
| Agent   | 停止该 Agent 的全部运行实例与其拥有的全部受管资源，写入 `closedAt`，发出 `agent.closed`         | `ACB.status = closed` |
| AOS     | 发出 `aos.stopping`，停止全部 system 级运行实例与受管资源，释放运行态注册表、事件订阅与缓存引用 | `stopped`             |

#### 恢复依据

恢复只依赖三种静态真相：AOSConfig、ACB / SCB 与 trace。缓存、hook 注册表、事件订阅、执行队列、运行实例、runtime resources 与 phase 都是可重建的运行事实。
...
| `skill.load`          | `name`                       | `SkillLoadResult`            | 否         | 通过 tool result 进入 trace             | 显式 load           |
| `skill.default.list`  | `scope`, `ownerId?`          | array of `DefaultSkillEntry` | 否         | 无                                      | 查询默认 skill 配置 |
| `skill.default.set`   | `scope`, `ownerId?`, `entry` | `revision`                   | 是         | live scope 可能触发缓存刷新或 reconcile | 设置默认 skill 条目 |
| `skill.default.unset` | `scope`, `ownerId?`, `name`  | `revision`                   | 是         | live scope 可能触发缓存刷新或 reconcile | 删除默认 skill 条目 |
...
| `agent.close`  | `agentId`      | `revision`     | 是         | 停止该 Agent 的运行实例与资源     | 关闭 Agent |
...
| `session.close`     | `sessionId`                       | `revision`                  | 是         | 停止该 Session 的运行实例与资源 | 关闭 Session     |

#### Resource 操作

| 操作             | 输入                        | 返回                              | 改写控制块 | 副作用                                     | 说明         |
| ---------------- | --------------------------- | --------------------------------- | ---------- | ------------------------------------------ | ------------ |
| `resource.list`  | `scope?`, `ownerId?`        | array of `ManagedRuntimeResource` | 否         | 无                                         | 列出受管资源 |
| `resource.start` | `scope`, `ownerId?`, `spec` | `ManagedRuntimeResource`          | 否         | 启动受管资源，触发 resource hooks / events | 创建运行资源 |
| `resource.get`   | `resourceId`                | `ManagedRuntimeResource`          | 否         | 无                                         | 查询资源     |
| `resource.stop`  | `resourceId`                | `resourceId`                      | 否         | 停止受管资源，触发 resource hooks / events | 关闭资源     |
...
公共 SDK 与 Control Plane 操作语义一一对应；运行中的 skill instance 拿到的是受 scope 约束的同构子集。CLI、SDK、HTTP 在底层共享同一套操作语义。
...
| 维度    | 修改命中 live scope 时      | 是否立即影响当前实例 / 上下文 | 未来何时消费                            |
| ------- | --------------------------- | ----------------------------- | --------------------------------------- |
| `load`  | 刷新该 scope 的默认文本缓存 | 否                            | bootstrap / post-compaction reinjection |
| `start` | 立即触发 reconcile          | 是                            | 同一请求处理流程内生效                  |

对于 non-live scope，只写控制块，不做 reconcile。
...
| 实例内部的 HTTP / shell 调用       | 仅当它们通过控制面影响 session 时才进入 trace |
| 宿主层面的缓存命中 / 失效          | 不进入                                        |

trace 的职责是记录会话的运行轨迹，不是替代宿主的全部运行日志系统。

恢复依赖的静态真相只有 AOSConfig、ACB / SCB 与 trace；运行态缓存、模块引用、hook 注册表、事件订阅、执行队列与 session phase 都必须在恢复后重新构造。bootstrap 的幂等恢复协议已在第六章交代，这里不再重复展开。

### 8.2 能力与运行资源的边界

memory、channel、notion、飞书、模拟社交平台，都属于 skill。某个 memory system 可以被替换为另一种 memory skill，某个 channel integration 也可以被另一种 channel skill 取代；AOS 提供的是统一的生命周期、控制面、trace、bus 与运行资源托管。

app、service 与 worker 属于受管 runtime resources。它们由 running skill instance 通过控制面请求创建，存在于运行时，受 scope 与 owner 约束，可被启动与停止。
...
Compute Unit 默认通过 `bash` 与现实世界交互；人类可以通过 UI 面板操作系统，后台服务可以通过 SDK 或 HTTP/API 接入，运行中的 skill 实例则通过 `AosSDK` 访问同一个控制面。`bash` 的单工具约束只作用于 Compute Unit，它保证 AI 侧入口保持单一，同时让系统治理继续收束在统一的控制面上。

Agent OS 是一个管理认知主体、会话运行轨迹、skill 注入与 skill 运行实例的认知操作系统。其本体层由 Compute Unit、Agent、Session、Skill 四层构成，静态控制结构由 AOSConfig、ACB、SCB 承载。会话运行轨迹以 `UIMessage[]` 风格记录，`metadata` 与 `data-*` parts 承载 AOS 语义，模型投影由 trace 转换而来。skill 的两种行为——load 对应上下文注入，start 对应实例化启动——由生命周期驱动，hooks 是实例在事件点上介入的正式机制，event bus 负责传播运行时事实。整个系统以 AOS Control Plane 作为唯一正式治理面，CLI、SDK 与 UI 均作为其客户端存在。
