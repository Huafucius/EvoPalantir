...
AdmissionHookCallback = Callable[[dict[str, Any], dict[str, Any]], Awaitable[None] | None]


@dataclass
class RegisteredAdmissionHook:
    instance_id: str
    owner_type: OwnerType
    owner_id: str | None
    callback: AdmissionHookCallback


class AdmissionHookEngine:
    def __init__(self) -> None:
        self._registry: dict[str, list[RegisteredAdmissionHook]] = {
            name: [] for name in ADMISSION_HOOK_SPECS
        }
    ...
    def register(
        ...
        owner_type: OwnerType,
        owner_id: str | None,
        hooks: dict[str, AdmissionHookCallback],
    ) -> list[str]:
        registered: list[str] = []
        for hook_name, callback in hooks.items():
            ensure_admission_hook_allowed(owner_type, hook_name)
            self._registry.setdefault(hook_name, []).append(
                RegisteredAdmissionHook(
    ...
    async def dispatch(
        ...
        *,
        ...
        spec = ADMISSION_HOOK_SPECS[hook_name]
    ...
    def _visible_hooks(
        ...
        *,
        agent_id: str | None,
        session_id: str | None,
    ) -> list[RegisteredAdmissionHook]:
        visible: list[RegisteredAdmissionHook] = []
